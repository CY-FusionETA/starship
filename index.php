...
// (Auth::refresh reads the REAL users table, so it must run before we flip to
// the demo database below — otherwise a tour would look for the user in the
// demo DB and sign them out.)
Auth::refresh();

// Guided tour / Training Mode: route every query to the isolated demo DB for
// the rest of this request. Nothing the learner does touches live data.
if (\App\Tour::active()) \App\Db::useDemo(true);

$r = new Router();

// --- Guided tour / Training Mode ------------------------------------
$r->post('/tour/start', function () {
    Auth::require();
    Csrf::check();
    \App\Tour::start((int)Auth::id(), (string)(Auth::user()['name'] ?? 'Trainee'), Auth::role());
    Response::redirect('/?tour=1');
});
$r->post('/tour/exit', function () {
    Auth::require();
    Csrf::check();
    \App\Tour::exit();
    Response::redirect('/');
